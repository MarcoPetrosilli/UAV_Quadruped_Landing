...
def save_and_plot(rows):
    ...
    # stimata dagli angoli comandati (roll/pitch in radianti, salvati PRIMA della
    # conversione rad2deg usata solo per il setpoint). Piccoli angoli: la spinta
    # verticale F fornisce circa G di sostegno, la sua componente orizzontale
    # proiettata dagli angoli da' l'accelerazione nel piano xy.
    # Convenzione coerente con send_setpoint(rad2deg(roll), rad2deg(pitch), 0, cmd):
    # pitch positivo -> accelerazione in +x, roll positivo -> accelerazione in -y.
    ...
    ax[6].set_ylabel("v_xy [m/s]"); ax[6].legend(loc="upper right"); shade_mpc(ax[6])
...
class LandingNode(Node):
    ...
    def tick(self):
        ...
        delta_eff = LOS_DELTA * min(1.0, elapsed / self.RAMP_T)
        ...
        if self.state == "landing" and self.dynamic_p_start is not None:
            p_end_land = self.WP[LANDING]
            seg = p_end_land - self.dynamic_p_start
            L = np.linalg.norm(seg)
            tau = time.perf_counter() - self.land_t0
            if L < 1e-6:
                p_LOS = p_end_land.copy()
            else:
                s_lin = min(L, self.V_LAND * tau)
                p_LOS = self.dynamic_p_start + (s_lin / L) * seg
        else:
            current_p_start = self.WP[self.old_wp_id]
            p_LOS, _ = LOS_wp(pos, current_p_start, self.WP[self.wp_counter],
                              delta=delta_eff, stop_delta=self.stop_delta)
        ...
        if distance <= self.stop_delta:
            if self.state == "rising":
                ...
                self.state = "landing"; self.old_wp_id, self.wp_counter, self.stop_delta = HOLD, LANDING, 0.1
                self.dynamic_p_start = self.WP[HOLD].copy()

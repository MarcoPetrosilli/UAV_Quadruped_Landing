...
# ---- calibrazione spinta -----------------------------------------------------
# HOVER_CMD e' FISSO per tutta la sessione di caratterizzazione: gli stimatori
# sotto girano solo in log e non lo modificano. Cosi' tutti i voli hanno lo
# stesso controllore e sono confrontabili tra loro.
# NB: dai voli del 14/9 l'hover vero a batteria carica e' ~53-54k. Con 49600
# nav/hold li regge il PID (integratore), ma nella fase MPC (senza integratore)
# manca ~7% di spinta: discesa piu' rapida e touchdown piu' duro del normale.
HOVER_CMD = 52000   # stesso valore della sessione con tutte le batterie in sequenza
MASS = 0.0379
HOVER_FORCE = MASS * G
...
def save_and_plot(rows):
    ...
    ax[9].axhline(HOVER_CMD, color="k", lw=1.2, ls=":", label=f"usato (fisso {HOVER_CMD})")
    ...
    for k, lab in (("hover_est", "attuale"), ("hover_b50", "B50"), ("hover_vbat", "vbat")):
        ...
        if len(v):
            print(f"hover {lab}: primo {v[0]:.0f}, ultimo {v[-1]:.0f} (fisso usato {HOVER_CMD})")
    if len(a["vbat"]):
...
class LandingNode(Node):
    ...
    def _update_hover_estimates(self, cmd_sent, z, vz):
        """Aggiorna gli stimatori dell'hover. SOLO LOG: HOVER_CMD non cambia.
    ...
    def tick(self):
        ...
        # stesso istante fisico. Solo log: HOVER_CMD resta fisso.

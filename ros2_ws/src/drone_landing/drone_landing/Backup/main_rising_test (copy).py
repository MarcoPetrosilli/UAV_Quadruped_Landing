...
#HOVER_CMD = 42100   # valore INIZIALE: viene ricalibrato a runtime dalla fase di
                    # hold (il cmd medio del PID in hold e' il vero hover del
                    # momento) prima che parta il landing MPC — vedi tick()
...
# ---- ricalibrazione runtime di HOVER_CMD dalla fase di hold ------------------
# In hold il PID (con integratore) porta il cmd al vero valore di hover del
# momento: lo accumuliamo e, all'ingresso in landing, aggiorniamo HOVER_CMD
# cosi' l'MPC (che NON ha integratore) parte con la calibrazione giusta per
# questa batteria/questo istante. Compensa la deriva tra run e a inizio run;
# NON quella durante il landing stesso (per quella servirebbe l'opzione 1).
...
TARGET_XY = np.array([0.0, -1.0])       # target parte qui
TARGET_VEL = np.array([0.0, 0.15, 0.0]) # si muove in +y (0.15 m/s; regola tu)
Z_CRUISE = 1.8
Z_LAND = 0.1
Z_HOLD = 1.8
ALPHA_CONE = 1.0
...
def save_and_plot(rows):
    ...
    fig, ax = plt.subplots(9, 1, sharex=True, figsize=(11, 14.5))
...
class LandingNode(Node):
    ...
    def __init__(self):
        ...
        self._hold_cmd_buf = []        # cmd raccolti durante hold, per ricalibrare HOVER_CMD
        self._hover_cmd_initial = HOVER_CMD   # valore di partenza (per il clamp di sicurezza)
    ...
    def tick(self):
        ...
        # accumula il cmd in hold: e' il vero hover del momento (PID con
        # integratore all'equilibrio) e serve per ricalibrare HOVER_CMD prima
        # del landing. Solo in hold "puro" (non emergenza).
        if self.state == "hold" and not self._emergency:
            self._hold_cmd_buf.append(cmd)
        ...
        if distance <= self.stop_delta and carrot_arrived:
            if self.state == "rising":
                ...
                # --- ricalibrazione runtime di HOVER_CMD dai cmd di hold ---
                # Il PID in hold ha portato il cmd al vero hover del momento:
                # aggiorniamo HOVER_CMD (globale usata da force_to_cmd) cosi'
                # l'MPC del landing parte con la calibrazione giusta

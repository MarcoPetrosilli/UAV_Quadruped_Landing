...
def plot_advanced_diagnostics(rows, stamp=None, alpha_cone=1.0, z_cut=1.0, r_base=0.1, polytope_path="reachable_polytope.npz"):
    ...
    # --- assi in scala reale (equal aspect): senza questo matplotlib auto-scala
    #     ogni asse in modo indipendente per riempire il box, e un movimento
    #     minuscolo in x (pochi cm) viene "stirato" quanto i metri in y,
    #     facendo APPARIRE oscillazioni che nella realta' sono 1-2cm. Uso lo
    #     stesso trattamento gia' presente nel plot del cono (max_range + box
    #     aspect 1:1:1) su tutti i punti disegnati (traiettoria + carota + target).
    ...
    max_range = max(xr[1]-xr[0], yr[1]-yr[0], zr[1]-zr[0], 1e-3) / 2.0
    xm = 0.5*(xr[0]+xr[1]); ym = 0.5*(yr[0]+yr[1]); zm = 0.5*(zr[0]+zr[1])
    ax3d.set_xlim(xm-max_range, xm+max_range)
    ax3d.set_ylim(ym-max_range, ym+max_range)
    ax3d.set_zlim(zm-max_range, zm+max_range)

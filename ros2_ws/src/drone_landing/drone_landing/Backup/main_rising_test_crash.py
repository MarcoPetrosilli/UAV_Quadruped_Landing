...
TARGET_VEL = np.array([0.1, -0.1, 0.0]) 
...
def save_and_plot(rows):
    ...
    ax[7].set_ylabel("a_xy [m/s^2]"); ax[7].legend(loc="upper right"); shade_mpc(ax[7])

    ax[8].plot(t, a["solve_ms"], color="tab:brown", lw=1.0)
    ax[8].set_ylabel("solve MPC [ms]"); ax[8].set_xlabel("t [s]"); shade_mpc(ax[8])
...
def plot_advanced_diagnostics(rows, stamp=None, alpha_cone=1.0, z_cut=1.0, r_base=0.1,
...
class RisingTestNode(Node):
    ...
    def tick(self):
        ...
        if self.state in ["landing"]:
            current_target_vel = TARGET_VEL

            # p_start deve muoversi ESATTAMENTE come p_end, altrimenti L (usata
            # dal profilo trapezoidale) cresce ogni tick invece di restare
            # costante, e la carota diventa via via piu' incoerente (il
            # problema che peggiorava verso la fine di "hold")
            
            #self.WP["hold"] += step_disp
            self.WP["landing"] += step_disp
            
            self.seg_p_start += step_disp
        else:

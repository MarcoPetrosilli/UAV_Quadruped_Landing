...
class HybridController:
    def __init__(self, g=9.81, dt=0.02, mass=0.029,
        ...
        # ---- HOVER-THRUST ESTIMATOR (adattivo) --------------------------
        # Stima F_hover in tempo reale dalla forza del PID quando il drone e'
        # vicino all'hover. L'MPC poi usa force = F_hover_hat + M*az_mpc invece
        # di M*(g+az): eredita cosi' la compensazione batteria/motori che il PID
        # fa via integratore, SENZA aggiungere un integratore all'MPC (resta
        # fedele a Persson) e SENZA dipendere da un HOVER_CMD fisso.
        # Disattivabile con HOVER_EST_ENABLED=False -> torna al comportamento
        # originale M*(g+az), per confronto A/B in sicurezza.
        self.HOVER_EST_ENABLED = True
        self.hover_force_hat = self.GRAVITY   # stima corrente [N], parte dal peso nominale
        self.hover_est_lambda = 0.02          # guadagno del filtro passa-basso (lento)
        self.hover_est_vz_thresh = 0.06       # aggiorna solo se |vz| < questo [m/s]
        self.hover_est_az_thresh = 0.6        # e se |az PID equivalente| < questo [m/s^2]
        self.hover_force_min = 0.5 * self.GRAVITY   # clamp di sicurezza (±50% dal peso)
        self.hover_force_max = 1.6 * self.GRAVITY
    ...
    def compute(self, cur_pos, cur_vel, target_pos, target_yaw=0.0,
        ...
        if gate_now or self.mpc_activated:
            ...
            # --- aggiorna l'hover estimator quando siamo vicini all'hover ---
            # In quasi-hover (vz piccola, accelerazione verticale richiesta
            # piccola) la forza del PID e' ~ il vero peso+bias del momento:
            # la filtriamo dentro hover_force_hat. Fuori dall'hover NON
            # aggiorniamo (la forza PID contiene anche l'accelerazione di
            # manovra, non solo la gravita').
            if self.HOVER_EST_ENABLED:
                vz = float(cur_vel[2])
                az_pid_equiv = force / self.M - self.g   # accel verticale implicita nella forza PID
                if abs(vz) < self.hover_est_vz_thresh and abs(az_pid_equiv) < self.hover_est_az_thresh:
                    lam = self.hover_est_lambda
                    self.hover_force_hat = (1.0 - lam) * self.hover_force_hat + lam * force
                    self.hover_force_hat = float(np.clip(
                        self.hover_force_hat, self.hover_force_min, self.hover_force_max))
    ...
    def _mpc_force_attitude(self, cur_pos, cur_vel, wp, target_yaw, a_xy_lim, target_vel):
        ...
        # Ancora di hover adattiva: F_hover_hat (stimato dal PID, compensa
        # batteria/motori) + l'incremento di forza per l'accelerazione MPC.
        # Equivale a M*(g+az) quando hover_force_hat == M*g (caso nominale),
        # ma segue le derive reali quando il PID le ha imparate.
        if self.HOVER_EST_ENABLED:
            force = self.hover_force_hat + self.M * az
        else:

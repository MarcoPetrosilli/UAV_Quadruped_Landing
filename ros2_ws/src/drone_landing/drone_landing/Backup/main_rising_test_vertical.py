...
Z_TEST = 1.8          # quota di prova [m]
Z_LAND = 0.1          # quota di "atterraggio" [m]
A_XY = 0.17
ALPHA_CONE = 1.0       # apertura del cono di landing (per i plot diagnostici)
R_BASE_CONE = 0.3      # raggio di base del cono (per i plot diagnostici)
...
def build_vbat_logconf():
    # blocco SEPARATO: il blocco "state" (6 float = 24 byte) e' gia' quasi al
    # limite di dimensione di un singolo blocco CRTP — aggiungere pm.vbat li'
    # dentro supera il tetto ("log configuration is too large"). Un blocco a
    # parte, anche piu' lento, evita il problema.
...
def save_and_plot(rows):
    ...
    fig, ax = plt.subplots(9, 1, sharex=True, figsize=(11, 15))
    ...
    ax[8].set_ylabel("batteria [V]"); ax[8].set_xlabel("t [s]")
    ax[8].legend(loc="upper right", fontsize=7)
...
def plot_advanced_diagnostics(rows, landing_target, stamp=None, alpha_cone=1.0, z_cut=1.0, r_base=0.1,
                               polytope_path="reachable_polytope.npz"):
    """Traiettoria 3D/2D, politopo del reachable-set e cono di landing.

    landing_target: np.array([x,y,z]) — target FISSO (nessun tracking per
    piattaforma mobile qui, coerente col resto del file). Usato al posto
    delle colonne target_x/y/z/vx/vy/vz (che main_alpha_landing.py ha per
    la piattaforma mobile e qui non servono)."""
    import numpy as np
    import os
    ...
    # target fisso: stesso valore per tutti i campioni, velocita' zero
    target_pos = np.tile(np.asarray(landing_target).reshape(3, 1), (1, len(rows)))
    target_vel = np.zeros((3, len(rows)))
    ...
    ax3d.scatter(*landing_target, color='k', marker='X', s=80, label="Landing target")
    ...
    ax2d.scatter(landing_target[0], landing_target[1], color='k', marker='X', s=80, label="Landing target")
    ...
    try:
        ...
        fig_poly.suptitle(f"Stato-errore sul set controllabile (target fisso) — {msg}")
    ...
    if np.sum(landing_mask) > 1:
        ...
        ax_cone.set_title(f"Traiettoria di landing nel cono (target fisso, alpha={alpha_cone}) — {status}")
...
class RisingTestNode(Node):
    def __init__(self):
        ...
        self.rows = []
        self.latest_vbat = float("nan")   # aggiornato dal blocco di log separato
    ...
    def tick(self):
        ...
        if self.state == "rising" and distance <= self.stop_delta and carrot_arrived:
            ...
            self.state = "landing"
        elif self.state == "landing" and pos[2] <= Z_LAND+1e-2:
            self.state = "idle"
            return
    ...
    def _start_emergency_landing(self):
        ...
        trapezoidale + gate MPC gia' usati per il landing normale, non e' logica
        nuova. Un secondo Ctrl+C durante questa fase fa comunque il taglio
        immediato (vedi main())."""
        if self._finished:
            return
        ...
        self.state = "landing"
        self.stop_delta = 0.05
        self.prev_state = None   # forza il reset del segmento (rampa) al prossimo tick
        self.get_logger().warn(
            f"ATTERRAGGIO DI EMERGENZA verso ({pos[0]:.2f}, {pos[1]:.2f}, {Z_LAND:.2f}) "
            f"— Ctrl+C di nuovo per taglio motori immediato.")
    ...
    def _shutdown_flight(self):
        ...
        if self.WP is not None:
            plot_advanced_diagnostics(rows=self.rows, landing_target=self.WP["landing"], stamp=stamp,
                                      alpha_cone=ALPHA_CONE, z_cut=1.0, r_base=R_BASE_CONE)
...
def main(args=None):
    ...
    try:
        ...
        try:
            ...
            # loop manuale invece di un secondo rclpy.spin(): anche col SIGINT
            # handler disattivato, meglio non ri-dipendere dall'executor ROS
            # per la fase critica — il comando ai motori passa da cflib
            # (radio/USB), indipendente da ROS.

...
class HybridController:
    def __init__(self, g=9.81, dt=0.02, mass=0.0379,
        ...
        # Il gate scambia le due leggi di controllo istantaneamente: senza
        # sfumatura il comando (forza/assetto) puo' avere un gradino netto
        # nel momento esatto dello switch (PID e MPC calcolano l'uscita in
        # modo completamente indipendente, nessuna continuita' garantita).
        # Si sfuma linearmente dall'ultima uscita PID all'uscita MPC per
        # blend_duration secondi, poi si passa alla MPC pura.
    ...
    def compute(self, cur_pos, cur_vel, target_pos, target_yaw=0.0,
        ...
        if gate_now or self.mpc_activated:
            ...
            if entering_mpc:
                ...
                # ancora di partenza per il bumpless transfer (vedi __init__)

"""
main_hover_test.py  —  TEST DELLO STIMATORE DI HOVER CON IL SOLO PID.

Stesso nodo di main_rising_test.py (mocap, decollo in anello aperto, logging,
stimatori), ma la missione e' ridotta a:
    prespin -> spool -> rising (verticale, sul posto) -> hold a HOVER_TEST_Z
    per HOVER_TEST_T secondi -> landing verticale col PID -> taglio motori.
L'MPC non viene mai attivato (landing=False sempre) e la piattaforma e' ferma.

Con ZERO_I_Z = True il guadagno integrale del PID sull'asse z e' 0: l'unico
feed-forward di spinta e' HOVER_CMD. Se HOVER_CMD e' sbagliato del dh%, il
termine P lo compensa con un errore di quota a regime:
    Kp * e_z = m*g * (h_vero/HOVER_CMD - 1)      ->  ~3 mm per ogni 1%
Quindi il drone NON cade anche con una stima sbagliata (con Kp=1.25 un errore
del 10% vale ~3 cm): la misura del test e' l'OFFSET di quota in hold, che il
mocap vede al decimo di millimetro, e da cui si ricava un hover "vero"
indipendente dall'accelerometro (figura *_hover_test.png).
Suggerito: un volo con HOVER_ADAPT = True e uno con False (valore fisso), a
parita' di batteria, per confrontare gli offset.

Sicurezza aggiunta rispetto al main di volo: watchdog sulla posa mocap
(POSE_TIMEOUT). Se la posa non si aggiorna, il controllo in anello chiuso si
ferma: sotto POSE_LOST_CUT_H si tagliano i motori, sopra si scende ad assetto
livellato regolando solo la vz stimata dal Kalman, poi si tagliano i motori.
"""
...
# B50 insegue lo stimatore B50 (rate limit HOVER_APPLY_RATE). Lo stimatore
# attuale e il modello su batteria restano solo in log.
...
# (troppo basso). B50 a inizio landing era entro ~100 unita' dal valore vero.
# HOVER_ADAPT = False riporta al comportamento di caratterizzazione (fisso).
HOVER_CMD_INIT = 52000     # valore usato fino all'aggancio B50 (salita e primi secondi)
HOVER_CMD = HOVER_CMD_INIT # valore APPLICATO, aggiornato a runtime (globale)
HOVER_ADAPT = True         # False = valore fisso HOVER_CMD_INIT per tutto il volo
...
HOVER_EST_CMD_MIN = 40000    # clamp di sicurezza sulla stima
...
# ---- TEST: parametri --------------------------------------------------------
HOVER_TEST_Z = 1.0         # quota di hover del test [m] (assoluta, come Z_CRUISE)
HOVER_TEST_T = 30.0        # durata dell'hold [s]
ZERO_I_Z     = True        # integrale del PID su z a 0 (feed-forward = solo HOVER_CMD)
TEST_V_LAND  = 0.3         # velocita' della carota in discesa [m/s]
LAND_CUT_H   = 0.06        # taglio motori sotto questa quota sopra il suolo [m]
POSE_TIMEOUT      = 0.25   # eta' massima della posa mocap [s]
POSE_LOST_CUT_H   = 0.30   # posa persa sotto questa quota (sul suolo): taglio [m]
POSE_LOST_VZ      = -0.3   # posa persa piu' in alto: discesa a questa vz [m/s],
POSE_LOST_KV      = 2.0    # regolata sulla vz del Kalman (solo IMU, valida per
                           # qualche secondo) con guadagno KV [1/s]
POSE_LOST_T_MARGIN = 1.5   # durata = quota/|vz| + margine, poi taglio [s]
HOLD_SKIP_T  = 5.0         # [s] di hold scartati nell'analisi (assestamento)

# ---- missione (ferma, verticale sul posto) -----------------------------------
TARGET_XY = np.array([0.0, 0.0])        # non usato nel test
TARGET_VEL = np.array([0.0, 0.0, 0.0])  # piattaforma ferma
Z_CRUISE = 1.8
Z_LAND = 0.1
Z_HOLD = 1.8
ALPHA_CONE = 1.0
...
def save_and_plot(rows):
    ...
    cols = ["t", "state", "mode", "x", "y", "z", "vx", "vy", "vz",
            ...
            "hover_est", "hover_b50", "b50_n", "hover_vbat",
            "az_mpc", "eps0", "eps_max", "z_ground"]
    ...
    ax[9].plot(t, a["hover_cmd"], color="k", lw=1.4, ls=":",
               label=f"applicato ({'B50' if HOVER_ADAPT else 'fisso'})")
    ...
    hv = np.concatenate([a[k][np.isfinite(a[k])] for k in ("hover_est", "hover_b50", "hover_vbat")]
    ...
    if len(eps_on):
        ...
        print(f"slack cono: max {eps_on.max():.4f} m, attivo (>1e-4) in {n_act}/{len(eps_on)} "
              f"tick MPC")
    for k, lab in (("hover_est", "attuale"), ("hover_b50", "B50"), ("hover_vbat", "vbat")):
    ...
    print(f"hover applicato: iniziale {HOVER_CMD_INIT}, finale {a['hover_cmd'][-1]:.0f} "
          f"(adattamento {'ATTIVO' if HOVER_ADAPT else 'spento'})")
    if len(a["vbat"]):
...
def plot_hover_test(rows, stamp, kp_z):
    """Hover implicito dall'offset di quota in hold (solo con ZERO_I_Z).

    A regime, con I=0 e velocita' nulla, il DSL-PID chiede
        F = m*g + Kp*e_z          (e_z = z_rif - z)
    e force_to_cmd lo converte con HOVER_CMD; la spinta vera e' F*HOVER_CMD/h_vero.
    Imponendo spinta vera = m*g:
        h_vero = HOVER_CMD * (1 + Kp*e_z/(m*g))
    Misura indipendente dall'accelerometro: usa solo mocap e comando applicato.
    """
    import matplotlib.pyplot as plt
    ...
    hold = st == "hold"
    if hold.sum() < 50:
        print("test: hold troppo corto per l'analisi")
        return
    th = t[hold]
    keep = th >= th[0] + HOLD_SKIP_T
    z, zr, hc = g("z")[hold], g("carrot_z")[hold], g("hover_cmd")[hold]
    e = zr - z
    e_s = np.convolve(e, np.ones(25) / 25, mode="same")          # ~0.5 s
    h_impl = hc * (1.0 + kp_z * e_s / (MASS * G))
    raw = g("hover_raw")[hold]
    raw_s = np.array(pd_rolling_median(raw, 50))
    b50, est = g("hover_b50")[hold], g("hover_est")[hold]

    fig, ax = plt.subplots(2, 1, sharex=True, figsize=(11, 7.5))
    ax[0].plot(th, e * 1000, color="0.6", lw=0.6, label="grezzo")
    ax[0].plot(th, e_s * 1000, "k", lw=1.5, label="media 0.5 s")
    ax[0].axhline(0, color="k", lw=0.6)
    ax[0].axvspan(th[0], th[0] + HOLD_SKIP_T, color="0.85", alpha=.6, label="scartato")
    ax[0].set_ylabel("z_rif - z [mm]")
    ax[0].set_title(f"Hold con I_z={'0' if ZERO_I_Z else 'attivo'}, adattamento "
                    f"{'B50' if HOVER_ADAPT else 'spento'}  (1% di errore ~ "
                    f"{MASS * G * 0.01 / kp_z * 1000:.1f} mm)")
    ax[0].legend(fontsize=8); ax[0].grid(alpha=.3)
    ax[1].plot(th, raw_s, color="0.5", lw=1.0, label="cmd/acc.z (mediana 1 s)")
    ax[1].plot(th, h_impl, "k", lw=1.6, label="hover implicito dall'offset")
    ax[1].plot(th, hc, "k:", lw=1.4, label="HOVER_CMD applicato")
    ax[1].plot(th, b50, color="tab:orange", lw=1.6, label="stimatore B50")
    ax[1].plot(th, est, color="tab:blue", lw=1.2, label="stimatore attuale")
    ax[1].set_ylabel("hover [cmd]"); ax[1].set_xlabel("t [s]")
    ax[1].legend(fontsize=8, ncol=3); ax[1].grid(alpha=.3)
    fig.tight_layout()
    png = f"last_run_plots/flight_{stamp}_hover_test.png"
    fig.savefig(png, dpi=110)

    k = keep
    em = float(np.median(e[k]))
    hi = float(np.median(h_impl[k]))
    print("\n==== TEST HOVER (hold, primi %.0f s scartati) ====" % HOLD_SKIP_T)
    print(f"offset quota mediano: {em*1000:+.1f} mm  (std {np.std(e[k])*1000:.1f} mm)")
    print(f"errore di calibrazione implicito: {100*(hi/np.median(hc[k])-1):+.2f} %")
    print(f"hover implicito {hi:.0f} | applicato {np.median(hc[k]):.0f} | "
          f"B50 {np.nanmedian(b50[k]):.0f} | attuale {np.nanmedian(est[k]):.0f} | "
          f"cmd/acc.z {np.nanmedian(raw[k]):.0f}")
    print(f"plot: {png}")


def pd_rolling_median(x, w):
    """Mediana mobile centrata che ignora i NaN (senza pandas)."""
...
class LandingNode(Node):
    ...
    def __init__(self):
        ...
        if ZERO_I_Z:
            # parametro, non modifica del DSL-PID: con I=0 il termine integrale
            # sparisce e integral_pos_e non ha piu' effetto sulla spinta
            self.ctrl.I_COEFF_FOR = self.ctrl.I_COEFF_FOR.copy()
            self.ctrl.I_COEFF_FOR[2] = 0.0
        self.latest_pose_t = None       # istante di arrivo dell'ultima posa
        self._pose_lost_t0 = None       # inizio discesa aperta per posa persa
        self.hold_t0 = None
        self.rows = []
        self.t_start = time.perf_counter()
        self.RAMP_T = 1.0
        
        self.V_LAND = TEST_V_LAND
    ...
    def _update_hover_estimates(self, cmd_sent, z, vz):
        ...
        # In landing le stime restano ferme al valore di hold: in discesa il
        # flusso entrante abbassa l'hover apparente di ~2% (volo 163915).
    ...
    def _log_extras(self, hover_raw):
        ...
        return dict(
            ...
            az_mpc=c.log_az_mpc, eps0=c.log_eps0, eps_max=c.log_eps_max,
    ...
    def tick(self):
        ...
        if self.state in ("prespin", "spool"):
            ...
            if self.state == "prespin":
                ...
                if vel[2] > LIFTOFF_VZ or pos[2] > self._z_ground + LIFTOFF_DZ:
                    ...
                    if self.ctrl.I_COEFF_FOR[2] > 0.0:
                    ...
                    # con I=0 nessun precarico: il distacco lo gestisce il solo P
                    self.state = "rising"
                    self._spool_t0 = None
        ...
        if self.WP is None:
            hx, hy = pos[0], pos[1]
            zg = self._z_ground if self._z_ground is not None else float(pos[2])
            self.WP = np.array([
                [hx, hy, pos[2]],          # start
                [hx, hy, HOVER_TEST_Z],    # rising: sale sul posto
                [hx, hy, HOVER_TEST_Z],    # nav: non usato
                [hx, hy, HOVER_TEST_Z],    # hold: fermo
                [hx, hy, zg],              # landing: carota fino al suolo
            ])
        ...
        landing = False   # TEST: solo PID, l'MPC non viene mai attivato
        ...
        if HOVER_ADAPT and np.isfinite(self.hover_b50):
            step = float(np.clip(self.hover_b50 - HOVER_CMD,
        ...
        zg = self._z_ground if self._z_ground is not None else Z_LAND
        if self.state == "landing" and pos[2] <= zg + LAND_CUT_H:
        ...
        if self.state == "rising" and distance <= self.stop_delta and carrot_arrived:
            self.state = "hold"; self.old_wp_id, self.wp_counter, self.stop_delta = RISING, HOLD, 0.3
            self.hold_t0 = time.perf_counter()
            self.get_logger().info(f"[TEST] hold a z={HOVER_TEST_Z:.2f} m per {HOVER_TEST_T:.0f} s")
        elif self.state == "hold" and time.perf_counter() - self.hold_t0 >= HOVER_TEST_T:
            ...
            self.get_logger().info("[TEST] fine hold -> landing (PID)")
    ...
    def _start_emergency_landing(self):
        ...
        zg = self._z_ground if self._z_ground is not None else Z_LAND
        self.WP[LANDING] = np.array([pos[0], pos[1], zg])
        self.state = "landing"
    ...
    def _shutdown_flight(self):
        ...
        try:
            plot_hover_test(self.rows, stamp, self.ctrl.P_COEFF_FOR[2])
        except Exception as e:
            self.get_logger().warn(f"plot del test fallito: {e}")

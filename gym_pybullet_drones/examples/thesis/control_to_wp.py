...
def run(
    ...
    start_pt = starting_pos
    end_pt = np.array([5.0, 5.0, 2.0])
    NUM_WP = 200
    TARGET_POS = np.linspace(start_pt, end_pt, num=NUM_WP) 
    ...
    #TARGET_POS[0, :] = [0.5, 0.5, 0.5]
    #TARGET_POS[1, :] = [0.5, 1.0, 0.75]
    #TARGET_POS[2, :] = [0.5, 1.75, 0.75]
    #TARGET_POS[3, :] = [1.25, 1.25, 0.75]
    #TARGET_POS[4, :] = [0.5, 0.5, 0.5]
    #TARGET_POS[5, :] = starting_pos
    ...
    for i in range(0, int(duration_sec*env.CTRL_FREQ)):
        ...
        #for j in range(num_drones):
        #    wp_counters[j] = wp_counters[j] + 1 if wp_counters[j] < (NUM_WP-1) and abs(pos_e) < 1e-3 else 0
        ...
        for j in range(num_drones):
            ...
            if wp_counters[j] == (NUM_WP - 1):
                arrived = True
            if wp_counters[j] == 0 and arrived:
            	arrived = False
                
            if distance < 0.2:
                wp_counters[j] = wp_counters[j] + 1 if not arrived else wp_counters[j] - 1

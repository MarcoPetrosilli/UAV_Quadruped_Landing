...
class MPCPIDHYControl(BaseControl):
    ...
    # ------------------------------------------------------------------ #
    def _mpc_position_control(self, cur_pos, cur_vel, wp, target_rpy, a_xy_lim):
        cur_x, cur_y, cur_z = cur_pos
        vx, vy, vz = cur_vel

        phi_cmd, theta_cmd = self._mpc_horizontal(cur_x, cur_y, vx, vy, wp, a_xy_lim)
    ...
    def _mpc_horizontal(self, cur_x, cur_y, cur_vx, cur_vy, wp, a_xy_lim):
        x = cp.Variable((4, self.N + 1))
        u = cp.Variable((2, self.N))
        xref = np.array([wp[0], wp[1], 0.0, 0.0])
        cost = 0
